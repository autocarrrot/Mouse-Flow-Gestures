...
* **Instant Window Management:** Move windows across desktops or pin them to all desktops using intuitive swipes.
* **Complete Media Control:** Play/Pause, Next/Prev track, and **Volume Control** entirely from your mouse.
* **Direct Desktop Switching:** Jump to any desktop instantly using `Win + Number` (e.g., Win+1 for Desktop 1).
* **Blender Mode:** Automatically disables middle-mouse gestures when Blender is active, preserving your 3D workflow.
...
*(Disabled automatically when Blender is active)*
...
| **`Win`+`Ctrl`+`Shift`+`→`** | Move active window to **Next Desktop**. (Auto-creates new desktop). |
| **`Win` + `1-9`** | Jump to Desktop 1-9. (If desktop doesn't exist, goes to last available). |
| **`Win` + `0`** | Jump to Desktop 10. |

### 5. Task View Shortcuts
*When the "Task View" overlay is open:*

* Press **`1-9`** or **`0`** to instantly switch to that desktop and close Task View.
